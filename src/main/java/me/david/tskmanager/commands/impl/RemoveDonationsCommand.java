...
public class RemoveDonationsCommand extends CommandModel {

	public RemoveDonationsCommand() {
		super("removedonation|rmdonate", "Removes an amount of credits from someone's donation amount", "removedonation|rmdonate {@member} {credits}");
		...
		if (args.size() == 3) {
			if (event.getMessage().getMentionedMembers().size() == 1) {
				try {
					...
					if (cache.getDonationLeaderboard().get(event.getMessage().getMentionedMembers().get(0).getEffectiveName()) != null)
						cache.getDonationLeaderboard().get(event.getMessage().getMentionedMembers().get(0).getEffectiveName()).removeCredits(credits);
					else {
						event.getChannel().sendMessage("You cannot remove credits from someone with no credits!").queue();
						return;
					}

					cache.serialize();
					event.getChannel().sendMessage("removed " + credits + " credits from " + event.getMessage().getMentionedMembers().get(0).getEffectiveName()).queue();
					...
					event.getChannel().sendMessage("Please provide the amount of credits you want to remove from that user!").queue();
				}
			} else
				event.getChannel().sendMessage("Please provide a member to remove credits from!").queue();
		} else
